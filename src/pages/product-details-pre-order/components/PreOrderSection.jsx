  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="flex items-center space-x-4">
          <div className="flex items-center border border-border rounded-organic">
            <button onClick={decrementQuantity} disabled={quantity <= 1} className="w-10 h-10 flex items-center justify-center"> <Icon name="Minus" size={16} /> </button>
            <span className="w-12 text-center font-medium">{quantity}</span>
            <button onClick={incrementQuantity} disabled={quantity >= 5} className="w-10 h-10 flex items-center justify-center"> <Icon name="Plus" size={16} /> </button>
          </div>
          <span className="text-sm text-text-secondary">Maximum 5 per order</span>
        </div>
      </div>
      ...
      <div>
        ...
        <div className="space-y-3">
          {paymentOptions.map(option => (
            <label key={option.id} className={`flex justify-between p-4 border rounded-organic ${selectedPayment === option.id ? 'border-primary bg-primary-50' : 'border-border'}`}>
              <div className="flex items-center space-x-3">
                <input
                  type="radio"
                  name="payment"
